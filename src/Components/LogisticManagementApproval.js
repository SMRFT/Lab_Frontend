...
const StatusBadge = styled.div`
  ...
  padding: 4px 8px;
  background-color: ${props => props.active ? '#ff9800' : '#4CAF50'};
  color: white;
  border-radius: 4px;
  margin-left: 10px;
  font-size: 14px;
...
const saveStartLocation = (position) => {
  ...
  axios.post('https://lab.shinovadatabase.in/sample_collector_location/', payload)
    .then(response => {
...
const updateEndLocation = (position) => {
  ...
  axios.put('https://lab.shinovadatabase.in/sample_collector_location/', payload)
    .then(response => {
      if (response.data.success) {
        
        // Update UI with distance from the response
        if (response.data.distance_travelled) {
          const distance = response.data.distance_travelled;
