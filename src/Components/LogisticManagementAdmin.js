...
const LocationModal = ({ isOpen, onClose, collectorName, collectorData }) => {
  ...
  const fetchCollectorLocation = async () => {
    try {
      ...
      const response = await axios.get(`https://lab.shinovadatabase.in/sample_collector_location/`, {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      intervalRef.current = setInterval(fetchCollectorLocation, 30000);
    }
          ...
          <>
            <LocationInfo>
              <InfoItem>
                ...
                <InfoValue style={{ color: '#16a34a', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  <Navigation size={14} />
                  Active
                </InfoValue>
              </InfoItem>
              <InfoItem>
                ...
                <InfoValue>{new Date(locationData.date).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}</InfoValue>
              </InfoItem>
              {locationData.distance_travelled && (
                <InfoItem>
                  <InfoLabel>Distance Travelled</InfoLabel>
                  <InfoValue>{locationData.distance_travelled} km</InfoValue>
                </InfoItem>
              )}
            </LocationInfo>
            ...
            <MapWrapper>
              <MapContainer 
                center={[locationData.latitudeStart, locationData.longitudeStart]} 
                zoom={13} 
              ...
              >
                ...
                />
                <Marker 
                  position={[locationData.latitudeStart, locationData.longitudeStart]}
                  icon={collectorIcon}
                >
                  <Popup>
                    {collectorName} <br />
            ...
            <div style={{ textAlign: 'center', color: '#64748b', fontSize: '0.875rem' }}>
              Location is automatically updated every 30 seconds
            </div>
          </>
        ) : (
          <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>
            No location data available for this collector.
          </div>
        )}
      </ModalContent>
    </ModalOverlay>
...
const LogisticManagementAdmin = () => {
  ...
  useEffect(() => {
    axios
      .get('https://lab.shinovadatabase.in/get_clinicalname/')
      .then((response) => {
