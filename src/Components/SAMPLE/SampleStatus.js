...
const SampleStatus = () => {
  ...
  useEffect(() => {
    const fetchPatientsByDate = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${Labbaseurl}sample_patient/`, {
    ...
    setPatients((prevPatients) =>
      prevPatients.map((patient) =>
        patient.patient_id === patientId
          ? {
              ...patient,
              tests: patient.tests.map((test, index) =>
                index === testIndex ? { ...test, status } : test
              ),
            }
          : patient
      )
  ...
  const saveAllTestsForPatient = async (patient) => {
    try {
      ...
      await axios.post(`${Labbaseurl}sample_status/`, [formattedData]);
      ...
      setSuccessMessage(
        `All test data saved successfully for ${patient.patientname}`
      );
      setError(null);
  ...
  const updateSelectedTests = async (patient) => {
    ...
    try {
      await axios.put(
        `${Labbaseurl}update_sample_status/${patient.patient_id}/`,
        updates
      );
                ...
                .map((patient) => (
                  <div key={patient.patient_id}>
                        ...
                        <tbody>
                          {patient.tests.map((test, index) => (
                            <Tr key={index}>
                              ...
                              <Td>
                                {testDetails[test.testname]?.department ||
                                  "N/A"}
                              </Td>
                              <Td>
                                <Select
                              ...
                              <Td>
                                <Checkbox
                                  ...
                                  onChange={() =>
                                    toggleSelectTest(patient.patient_id, index)
                                  }
                                />
                              </Td>
                    ...
                    <ButtonGroup>
                      <Button
                        success
                        onClick={() => saveAllTestsForPatient(patient)}
                      >
                        <Check size={16} />
                        Save All
                      </Button>
                      <Button
                        primary
                        onClick={() => updateSelectedTests(patient)}
                      >
