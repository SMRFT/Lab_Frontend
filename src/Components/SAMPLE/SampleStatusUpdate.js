...
const SampleStatusUpdate = () => {
  ...
  useEffect(() => {
    const fetchSampleCollected = async () => {
      setLoading(true)
      try {
        const response = await axios.get(`${Labbaseurl}get_sample_collected/`)
  ...
  const updateTestStatus = async (patientId, testIndex) => {
    const updatedStatus = statusChanges[selectedPatient.patient_id]?.[testIndex]
    ...
    try {
      const response = await axios.put(`${Labbaseurl}update_sample_collected/${patientId}/`, {
        updates: [
          {
            testIndex,
            ...
            sample.patient_id === patientId
              ? {
                  ...
                  testdetails: sample.testdetails.map((detail, idx) =>
                    idx === testIndex
                      ? { ...detail, samplestatus: updatedStatus, remarks: updatedRemarks || null }
                      : detail,
                  ),
                }
              : sample,
            ...
            <>
              {filteredPatients.length > 0 ? (
                    ...
                    <tbody>
                      {filteredPatients.map((sample) => (
                        <Tr key={sample.patient_id}>
                          ...
                          <Td>
                            <Button primary onClick={() => openModal(sample.patient_id)}>
                              <FileText size={16} />
                              View Details
                            </Button>
                          </Td>
                ...
                <EmptyState>
                  ...
                  <EmptyStateText>No samples found with the status "Sample Collected".</EmptyStateText>
                </EmptyState>
              )}
            </>
          )}
            ...
            <ModalContent>
              ...
              <PatientInfoCard>
                ...
                <PatientInfoItem>
                  ...
                  <PatientInfoValue>{selectedPatient.patient_id}</PatientInfoValue>
                </PatientInfoItem>
                <PatientInfoItem>
                  <Calendar size={16} />
                  ...
                  <tbody>
                    {selectedPatient.testdetails.map((detail, testIndex) => (
                      <Tr key={testIndex}>
                        ...
                        <Td>
                          <Select
                            value={statusChanges[selectedPatient.patient_id]?.[testIndex] ?? detail.samplestatus}
                            onChange={(e) => handleStatusChange(selectedPatient.patient_id, testIndex, e.target.value)}
                          >
                        ...
                        <Td>
                          {statusChanges[selectedPatient.patient_id]?.[testIndex] === "Rejected" && (
                            <Textarea
                              value={remarks[`${selectedPatient.patient_id}-${testIndex}`] || ""}
                              onChange={(e) =>
                                handleRemarksChange(selectedPatient.patient_id, testIndex, e.target.value)
                              }
                        ...
                        <Td>
                          <Button
                            success={!savedTests[`${selectedPatient.patient_id}-${testIndex}`]}
                            secondary={savedTests[`${selectedPatient.patient_id}-${testIndex}`]}
                            onClick={() => updateTestStatus(selectedPatient.patient_id, testIndex)}
                            disabled={savedTests[`${selectedPatient.patient_id}-${testIndex}`]}
                          >
                            {savedTests[`${selectedPatient.patient_id}-${testIndex}`] ? (
                              <>
                                <CheckCircle size={16} />
                                Updated
                              </>
