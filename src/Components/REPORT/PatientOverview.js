...
const PatientOverview = () => {
  ...
  useEffect(() => {
    axios.get(`${Labbaseurl}clinical_name/`)
      .then((response) => {
  ...
  const handleDispatch = async (patient) => {
    try {
      ...
      if (response.status === 200) {
        alert(`Dispatch updated successfully for Patient: ${patient.patient_name}`);
        // Refresh the data
        const formattedStartDate = startDate.toISOString().split("T")[0];
        const formattedEndDate = endDate.toISOString().split("T")[0];
  ...
  const handlePrint = async (patient, withLetterpad = true) => {
    try {
      ...
      const calculateTotalPages = () => {
        ...
        const footerStartY = pageHeight - footerHeight - signatureHeight - (withLetterpad ? 0 : disclaimerHeight)
      ...
      const addPageHeaderAndFooter = (pageNum) => {
        ...
        doc.setFontSize(8)
        doc.text(`Page ${pageNum} of ${totalPages}`, rightMargin - 5, 5, { align: "right" })
      }
      ...
      const checkForNewPage = (yPos, estimatedHeight) => {
        ...
        const footerStart = pageHeight - (footerHeight + safetyMargin + (withLetterpad ? 0 : disclaimerHeight))
      ...
      const addDisclaimer = (yPos) => {
        if (!withLetterpad) {
          ...
          doc.text("24, Saradha College Road, Salem-636007, Tamilnadu", leftMargin, yPos + 5)
      ...
      const addSignatures = () => {
        const pageHeight = doc.internal.pageSize.height
        const signaturesY = pageHeight - footerHeight - signatureHeight + 10 - (withLetterpad ? 0 : disclaimerHeight)
      ...
      for (let i = 0; i < leftDetails.length; i++) {
        ...
        if (right) {
          ...
          if (right.label === "Patient Ref.No" && patientRefNoNumber !== "N/A" && barcodeImage) {
            doc.addImage(barcodeImage, "PNG", rightValueX + doc.getTextWidth(right.value) - 10, currentYPosition + 2, 25, 8);
          }
        }
        ...
        Object.keys(testsByDepartment).forEach((department) => {
          ...
          testsByDepartment[department].forEach((test) => {
            ...
            testsToRender.forEach((currentTest, index) => {
              ...
              const methodLines = doc.splitTextToSize(methodText, colWidths[1] - 2); // adjust padding
      ...
      const pageHeight = doc.internal.pageSize.height
      const footerStart = pageHeight - (footerHeight + signatureHeight + 5 + (withLetterpad ? 0 : disclaimerHeight))
      ...
      doc.setFont("helvetica", "bold")
      const centerX = leftMargin + contentWidth / 2
      doc.text("**End Of Report**", centerX, currentYPosition, { align: "center" })
  
      // Verification - FIXED: Align with left margin
      const yPosition = currentYPosition + 10
      doc.setFont("helvetica", "normal")
      doc.setFontSize(9)
      doc.text(`Verified by: ${patientDetails.verified_by}`, leftMargin, yPosition)
