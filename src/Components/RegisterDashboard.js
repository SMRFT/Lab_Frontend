...
const PatientDashboard = () => {
  ...
  const fetchData = async (selectedDate) => {
    setLoading(true)
    try {
      const response = await axios.get(`https://lab.shinovadatabase.in/patients-by-date/?date=${selectedDate}`)
  ...
  const exportToCSV = () => {
    ...
    // Create CSV headers
    const headers = ["Bill No", "Patient Name", "Test Count", "Billing Status", "Total Amount"]
  ...
  const totalTests = patients.reduce((sum, p) => sum + (Number.parseInt(p.test_count) || 0), 0)
  const totalAmount = patients.reduce((sum, p) => sum + (Number.parseFloat(p.totalAmount) || 0), 0).toFixed(2)
  const billedPatients = patients.filter((p) => Number.parseFloat(p.totalAmount) > 0).length
      ...
      <TableContainer>
        <TableHeader>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {patients.length > 0 && (
              <Button onClick={exportToCSV} style={{ backgroundColor: "#10b981" }}>
                <Download size={16} />
                Export CSV
              </Button>
            )}
                  ...
                  <Td>
                    <span
                      style={{
                        ...
                        fontSize: "0.75rem",
                        backgroundColor: Number.parseFloat(p.totalAmount || 0) > 0 ? "#dcfce7" : "#fee2e2",
                        color: Number.parseFloat(p.totalAmount || 0) > 0 ? "#166534" : "#b91c1c",
                        fontWeight: "500",
                      }}
                    >
                      {Number.parseFloat(p.totalAmount || 0) > 0 ? "Billed" : "Not Billed"}
